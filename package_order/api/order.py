...
def create_shiprocket_order(doc, method=None):
    ...
    try:
        ...
        # -----------------------------
        email = address.email_id or doc.contact_email

        if not is_valid_email(email):
            # fallback
            email = "support@quantumberg.com"

        # DEBUG LOG
        frappe.log_error(f"Shiprocket Email Used: {email}")
        ...
        if not phone:
            frappe.throw(f"Invalid Phone: {raw_phone}")
